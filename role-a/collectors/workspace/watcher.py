...
def should_capture(path: Path, root: Path, max_bytes: int = 10 * 1024 * 1024) -> bool:
...
class WorkspaceHandler(FileSystemEventHandler):
    def __init__(self, root: Path, emit: Callable[[dict[str, object]], None], debounce_seconds: float = 2.0) -> None:
...
def run(endpoint: str, workspaces: list[Path]) -> None:
    observer = Observer()
    for workspace in workspaces:
        ...
        observer.schedule(WorkspaceHandler(root, lambda item: safe_post(endpoint, item)), str(root), recursive=True)
    observer.start()
    try:
        while True:
            time.sleep(1)
