...
async function deliver(event) {
  ...
  if (!response.ok) throw new Error("Intent server returned " + response.status);
}
...
async function handleMessage(message, sender) {
  ...
  if (message.kind === "intent-detailed-capture-status") {
    const config = await getDetailedConfig();
  ...
  if (message.kind !== "intent-user-action" || !(await detailedCaptureEnabled())) {
