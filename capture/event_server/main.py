"""FastAPI entry point for the local Intent Role A service."""
...
def _source_staleness_seconds() -> int:
    try:
        value = int(os.environ.get("INTENT_SOURCE_STALE_AFTER_SECONDS", "1800"))
    except ValueError:
        return 1800
...
def _writer_settings(
    *,
    ...
    return {
        ...
        else _environment_int("INTENT_INGEST_QUEUE_CAPACITY", 1_024),
        "max_batch_size": batch_size
        if batch_size is not None
        else _environment_int("INTENT_INGEST_BATCH_SIZE", 100),
        "max_batch_wait_ms": batch_wait_ms
        if batch_wait_ms is not None
        else _environment_int("INTENT_INGEST_BATCH_WAIT_MS", 25, minimum=0),
        "submit_timeout_seconds": submit_timeout_seconds
        if submit_timeout_seconds is not None
        else _environment_int("INTENT_INGEST_SUBMIT_TIMEOUT_SECONDS", 5),
    }
...
def create_app(
    ...
    *,
    ...
    app = FastAPI(title="Intent Event Server", version="1.0.1")
    ...
    extension_origin = os.environ.get("INTENT_FIREFOX_EXTENSION_ORIGIN")
    ...
    @app.get("/v1/export/day/meta", response_model=DayExportMeta)
    def export_day_meta(date: str, store: EventStore = Depends(get_store)) -> DayExportMeta:
        try:
            return store.export_day_meta(date)
        except ValueError as exc:
            raise HTTPException(status_code=422, detail=str(exc)) from exc

    @app.get("/v1/events/incremental", response_model=IncrementalEventsPage)
    def events_incremental(
        date: str,
        store: EventStore = Depends(get_store),
        after_ts: int | None = Query(default=None, ge=0),
        after_id: str | None = Query(default=None),
        limit: int = Query(default=500, ge=1, le=2000),
    ) -> IncrementalEventsPage:
        try:
            return store.list_events_incremental(date, after_ts=after_ts, after_id=after_id, limit=limit)
        except ValueError as exc:
            raise HTTPException(status_code=422, detail=str(exc)) from exc
    ...
    app.add_api_route("/export/day/meta", export_day_meta, methods=["GET"], response_model=DayExportMeta)
    app.add_api_route("/events/incremental", events_incremental, methods=["GET"], response_model=IncrementalEventsPage)
