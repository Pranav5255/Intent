...
from intent_engine.llm_settings import (
...
def create_app(store: IntentStore | None = None, role_a_client: RoleAClient | None = None) -> FastAPI:
    ...
    application = FastAPI(title="Intent Engine", version=API_VERSION)
    ...
    role_a_base_url = os.environ.get("INTENT_CAPTURE_URL", "").strip() or "http://127.0.0.1:9477"
    application.state.role_a_client = role_a_client or RoleAClient(role_a_base_url)

    def reload_llm_clients() -> None:
    ...
    @application.post("/settings/llm/gemini-credentials")
    async def upload_gemini_credentials(credentials: dict[str, object] = Body(...)) -> dict:
        raw = credentials.get("credentials")
        if not isinstance(raw, str) or not raw.strip():
            raise HTTPException(status_code=400, detail="credentials must be a non-empty JSON string")
        try:
            summary = save_gemini_credentials(raw.encode("utf-8"))
            reload_llm_clients()
            return summary
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc

    @application.delete("/settings/llm/gemini-credentials")
    async def delete_gemini_credentials() -> dict:
        summary = clear_gemini_credentials()
        reload_llm_clients()
        return summary
