...
from intent_engine.schemas import DayExport, DayExportMeta, EventCursor, IncrementalEventsPage, RawEvent
...
class RoleAClient:
    ...
    async def fetch_export_meta(self, date: str) -> DayExportMeta:
        """Fetch cheap day-export metadata for change detection."""

        self._validate_iso_date(date)
        data = await self._get_json("/v1/export/day/meta", params={"date": date})
        try:
            return DayExportMeta.model_validate(data)
        except ValidationError as exc:
            raise ValueError("Role A returned invalid day export metadata") from exc
    ...
    async def fetch_events_incremental(
        ...
        *,
        ...
        """Fetch one paginated slice of a day's events after an optional cursor."""

        self._validate_iso_date(date)
        params: dict[str, Any] = {"date": date, "limit": limit}
        if cursor is not None:
            params["after_ts"] = cursor.after_ts
            params["after_id"] = cursor.after_id
        data = await self._get_json("/v1/events/incremental", params=params)
        try:
            return IncrementalEventsPage.model_validate(data)
        except ValidationError as exc:
            raise ValueError("Role A returned an invalid incremental events page") from exc
    ...
    async def _get_json(self, path: str, *, params: dict[str, Any] | None = None) -> Any:
        ...
        if response.status_code == httpx.codes.NOT_FOUND and "/export/day/meta" in path:
            raise RoleAUnavailableError("Role A export metadata endpoint is unavailable")
