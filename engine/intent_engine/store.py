...
from intent_engine.schemas import Intent, PipelineResult, RawEvent
...
class IntentStore:
    ...
    def init_schema(self) -> None:
        ...
        try:
            connection.executescript(
                ...
                CREATE TABLE IF NOT EXISTS ingest_day_events (
                    date TEXT NOT NULL,
                    id TEXT NOT NULL,
                    ts INTEGER NOT NULL,
                ...
                CREATE INDEX IF NOT EXISTS idx_ingest_day_events_order
                    ON ingest_day_events(date, ts, ingested_at, id);
                """
            )
            try:
                connection.execute(
    ...
    async def upsert_ingest_day_events(self, date: str, events: list[RawEvent]) -> None:
        if not events:
            return
        async with self._connection() as connection:
            try:
                ...
                await connection.executemany(
                    """
                    INSERT INTO ingest_day_events(date, id, ts, ingested_at, event_json)
                    VALUES (?, ?, ?, ?, ?)
                    ...
                    [
                        (
                            ...
                            event.id,
                            event.ts,
                            event.ingested_at if event.ingested_at is not None else event.ts,
    ...
    async def list_ingest_day_events(self, date: str) -> list[RawEvent]:
        async with self._connection() as connection:
            cursor = await connection.execute(
                """
                SELECT event_json FROM ingest_day_events
                WHERE date = ?
                ORDER BY ts ASC, ingested_at ASC, id ASC
                """,
                (date,),
            )
            rows = await cursor.fetchall()
        return [RawEvent.model_validate_json(row["event_json"]) for row in rows]

    async def delete_ingest_day_events(self, date: str) -> None:
        async with self._connection() as connection:
            await connection.execute("DELETE FROM ingest_day_events WHERE date = ?", (date,))
            await connection.commit()

    async def get_ingest_day_metadata(self, date: str) -> str | None:
        return await self.get_metadata(f"ingest_day:{date}")
