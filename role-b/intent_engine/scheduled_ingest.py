    ...
    *,
    ...
    for date_value in dates:
        ...
        try:
            ...
            return ScheduledIngestResult(True, OUTCOME_ROLE_A_UNAVAILABLE, tuple(processed))
        except Exception:
            ...
            return ScheduledIngestResult(True, OUTCOME_PIPELINE_ERROR, tuple(processed))
    ...
    return ScheduledIngestResult(True, OUTCOME_SUCCESS, tuple(processed))
...
def main() -> int:
    ...
    logger = DiagnosticsLogger(str(Path(database_path).with_name("role-b-scheduled-ingest.jsonl")))
    result = asyncio.run(
    ...
    return 0 if result.outcome == OUTCOME_SUCCESS else 1
