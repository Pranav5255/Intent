...
| Resume | `intent_engine/resume.py` | Cluster -> `ResumePayload`; parent payload is rebuilt from the full session chronology. | Most-recent editor files are capped at 5; sanitized HTTP(S) browser URLs retain the final observed URL for each tab and are capped at 8; shell keeps recent `cwd`/`last_cmd`. No restore action is performed. |
